...
host = "https://fudi.cvmart.net/api/register"
headers={
...
def register(username):
    while True:
        ...
        #获取图片KEY值
        captcha_url = "https://fudi.cvmart.net/api/register-captcha"
        a1 = requests.get(captcha_url)
        key = a1.json().get("key")
        phone_url = "https://fudi.cvmart.net/api/register-sms"
        params = {
            "mobile":phone,
            "captcha_value":30,
            "key":key
...
def login_desk(username):
    """登录成功，修改配置的headers"""
    login_url = "https://fudi.cvmart.net" + '/api/login'
    data = {
        'username': username,
        'password': '123456',
    }
...
def email_true(username):
    ...
    url = "https://fudi.cvmart.net/api/user/base/bind-email/verification-code?email=asdas@qq.co"
    params={"email":email}
    code = requests.get(url,params=params,headers=headers).json()
    email_code = code.get("email_code")
    url1 = "https://fudi.cvmart.net/api/user/base/bind-email"
    data = {
        "email":email,
