...
class XLPT_Train(Zheng):
    ...
    #上传代码 #http://cvmart-11:0r0RV2gzMxgDxUV4@10.229.0.5:80/cvmart-11/project-1-instance-11-train-code
    def push_git_code(self):
        ...
        git_train_path = user_message.get('data').get('gitlab_path')[7:].replace("10.229.0.5:80","36.155.14.157")
        git_ev_sdk_path = user_message.get('data').get('encoding_gpu_containers')[7:].replace("10.229.0.5:80","36.155.14.157")
    ...
    """报名"""
    def xu_apply(self):
        print(self.apply_code)
        if self.apply_code == True:
            url = Web_Config.web_host+Web_Config.baoming_url.format(project_id=self.project_id)
            data = {
            "actual_name":"asdasdas",
            "id_number":"12321321",
            "email":self.email,
            "check":True
            }
    ...
    def creat_train(self):
        ...
        data = {
         "exec_command":"bash /project/train/src_repo/start_train.sh 1000"
        }
        code = requests.post(url,data=data,headers=self.headers).json()
