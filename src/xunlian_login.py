...
class Zheng():
    ...
    def get_id(self):
        ...
        project_id = code.get("list")[0].get("id")
        return project_id
    """获取邮箱"""
    def get_email(self):
    ...
    def get_apply(self):
        url = Web_Config.web_host + Web_Config.get_project_id
        code = requests.get(url,headers=self.headers).json()
        status_operate_zh = code.get("list")[1].get("status_operate_zh")
        # print(status_operate_zh)
        if status_operate_zh == "报名中":
            return True
        else:
            return False
    ...
    def stop_train(self):
        ...
        if code.get("list")[0].get("status_cn") == "排队中":
            url = "http://developer.aiebuy.cn/api/online-train/instance/train/%s/revoked" % code.get("list")[0].get("id")
            a1 = requests.patch(url, headers=self.headers)
        elif code.get("list")[0].get("status_cn") != "训练中":
            url = "http://developer.aiebuy.cn/api/online-train/instance/train/%s/aborted" % code.get("list")[0].get("id")
            a1 = requests.patch(url, headers=self.headers)
    ...
    def stop_test(self):
        ...
        code = requests.get(url,headers=self.headers).json()
        if code.get("list")[0].get("status_cn") == "排队中":
            url = "http://developer.aiebuy.cn/api/online-train/instance/sdk-test/%s/revoked" % code.get("list")[0].get("id")
            a1 = requests.patch(url, headers=self.headers)
        elif code.get("list")[0].get("status_cn") != "测试中":
            url = "http://developer.aiebuy.cn/api/online-train/instance/sdk-test/%s/aborted" % code.get("list")[0].get("id")
            a1 = requests.patch(url, headers=self.headers)
    ...
    def get_git_path(self):
        ...
        try:
            codes = requests.get(url,headers=self.headers_admin,params=params).json().get("data")
            for code in codes:
